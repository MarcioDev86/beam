...
import { Dropdown, DropdownToggle, DropdownMenu, DropdownItem, UncontrolledAlert } from 'reactstrap';
...
function Brand() {
    ...
    const [{brandState, brandName, deviceName, codeText, codeState}, dispatch] = React.useContext(MyContext);
    ...
    const getCodes = (brand) => {
        return (
            codes.map(p => {return p.brand === brand && p.device === deviceName ? <UncontrolledAlert  style={{backgroundColor: "#ECE5EE"}}>{p.code}</UncontrolledAlert> : null})
        )
    }
    ...
    const createBrandItems = () => {
        switch(deviceName){
            case "SELECT YOUR DEVICE":
              return <DropdownItem className="dpItem">SELECT A DEVICE FIRST!</DropdownItem>
            
            case "AUDIO":
              return (
                audiocodes.AUDIO.map( t => (
                  <DropdownItem className="dpItem" onClick={() => ShowCodes(t)} key={t}>{t}</DropdownItem>
                ))
              )
      
            case "BLU-RAY":
              return (
                blucodes.BLURAY.map( t => (
                  <DropdownItem className="dpItem" onClick={() => ShowCodes(t)} key={t}>{t}</DropdownItem>
                ))
              ) 
            ...
            case "DIGITAL FRAME":
              return (
                digitalframecodes.DIGITALFRAME.map( t => (
                  <DropdownItem className="dpItem" onClick={() => ShowCodes(t)} key={t}>{t}</DropdownItem>
                ))
              )
            ...
            case "STREAMING MEDIA PLAYER":
              return (
                streamingcodes.STREAMING.map( t => (
                  <DropdownItem className="dpItem" onClick={() => ShowCodes(t)} key={t}>{t}</DropdownItem>
                ))
              )
    ...
    return (
        <div>
            <Dropdown isOpen={brandState} toggle={() => setBrandState()}>
                ...
                <DropdownMenu style={{width : "100%", overflowY: "auto", maxHeight: "50vh"}} onChange={() => createBrandItems()}>
                    {createBrandItems()}
                </DropdownMenu>
            </Dropdown>
        </div>
